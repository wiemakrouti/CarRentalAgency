...
- [x] **Phase 3** — Clients module: CRUD (soft-delete + restore), Cloudinary document upload/delete (ID/license/passport scans, no set-primary — `ClientDocument` has no such field), blacklist flag, audit logging (`CREATE`/`UPDATE`/`DELETE`/`RESTORE`/`CLIENT_DOCUMENT_*`), server-side list/filter/search/pagination via TanStack Query. See `docs/api.md` and `docs/architecture.md`.
