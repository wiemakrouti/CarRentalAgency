...
- **Clients** — `GET/POST /clients`, `GET/PATCH/DELETE /clients/:id`, `GET /clients/:id/deletable`, `POST /clients/:id/documents` (multipart, field `document` + `type`), `DELETE /clients/:id/documents/:documentId`. Client has no soft-delete (see `docs/architecture.md` § Soft delete): `DELETE /clients/:id` hard-deletes for real, guarded by `GET /clients/:id/deletable` → `{ canDelete, reason }` — blocked (`409 CLIENT_HAS_HISTORY`) if the client has any rental, even a cancelled one. Duplicate `email` → `409 DUPLICATE_EMAIL`. Same image constraints as Cars (5MB, JPEG/PNG/WebP) via the same shared upload middleware. `GET /clients`/`GET /clients/export` also accept `sortBy` (`lastName`/`city`/`createdAt`/`drivingLicenseExpiry`, default `createdAt`), `sortOrder` (`asc`/`desc`), `city`, and `licenseStatus` (`expired`/`expiring`/`ok`/`not_set`, same 30-day window as the frontend's expiry badges). `GET /clients/check-phone?phone=&excludeId=` — non-blocking duplicate-phone lookup for the form (a phone number can legitimately be shared, e.g. family members, so this only warns). `GET /clients/:id/stats` — `{ totalRentals, completedRentals, totalRevenue, lastRentalDate, onTimeReturnRate }` for the profile panel, same `totalRevenue` definition as Cars/Finances. Each client in the `GET /clients` list also carries `reliabilityRate` (share of resolved — `COMPLETED`+`CANCELLED` — rentals actually honored, `null` with no resolved rental yet) plus the underlying `completedRentals`/`cancelledRentals` counts, computed for the whole page in one grouped query rather than re-running `getStats` per row. (A `blacklisted` flag existed early on but was removed — it had no enforcement anywhere and no real usage; `notes` covers "flag a problem client" instead. Likewise `ClientDocument.expiryDate` was removed — unlike the driving license expiry, nothing ever alerted on it.)
- **Rentals** — `GET /rentals`, `GET /rentals/:id`, `POST /rentals`. Creation validates the car is `status: AVAILABLE` *and* has no date-overlapping `RESERVED`/`ACTIVE` rental (shared overlap definition in `backend/src/lib/rental-availability.ts`, also used by `/cars/available`) → both failure modes return `409 CAR_NOT_AVAILABLE` with a message describing which. `totalAmount` = the car's current `dailyRate` × nights, snapshotted onto the rental (never re-read from `Car` later). `depositAmount` defaults from the `Setting` singleton unless the request overrides it. `rentalNumber` is server-generated (`LOC-YYYYMMDD-XXXX`), retried up to 5 times on the practically-impossible unique-constraint collision. List/get responses include `extensions` and `payments` (Phase 4b) alongside `car`/`client`.
