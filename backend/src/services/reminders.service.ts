...
export class RemindersService {
  static async getUpcoming(agencyId: string, withinDays = 7): Promise<Reminder[]> {
    ...
    const [
      ...
      // documents below. Scoped to this agency's own cars, not carId
      // directly (MaintenanceRecord has no agencyId of its own).
      prisma.maintenanceRecord.findMany({
        where: { deletedAt: null, nextDueDate: { not: null, lte: horizon }, car: { agencyId } },
      }),
